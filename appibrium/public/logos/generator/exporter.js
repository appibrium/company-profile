...
async function run() {
  ...
  for (const { name, scale } of wmScales) {
    ...
    // Clean No-Tag Wordmark (Light backgrounds - Dark Text, Green a)
    const cleanLightHtml = `
      ${BASE_HTML_HEAD}
      <style>${wmCSS}</style>
      <div id="target" class="wm-custom">
        <div class="name-custom" style="color: var(--night);"><span style="color: var(--mintlo);">a</span>ppibrium</div>
    ...
    await exportElement(
      cleanLightHtml,
      '#target',
      path.join(OUTPUT_DIR, 'wordmark', `wordmark_notag_dark_on_transparent_${name}.png`),
      Math.round(500 * scale),
      Math.round(150 * scale)
    );

    // Clean No-Tag Wordmark (Dark backgrounds - Light Text, Mint a)
    const cleanDarkHtml = `
      ${BASE_HTML_HEAD}
      <style>${wmCSS}</style>
      <div id="target" class="wm-custom">
    ...
    await exportElement(
      cleanDarkHtml,
      '#target',
      path.join(OUTPUT_DIR, 'wordmark', `wordmark_notag_light_on_transparent_${name}.png`),
      Math.round(500 * scale),
      Math.round(150 * scale)
    );
  }
